...
test("placement evidence requires at least eighty percent across multiple questions", () => {
  assert.equal(passesEvidenceThreshold(4, 5), true);
  assert.equal(passesEvidenceThreshold(3, 5), false);
  assert.equal(passesEvidenceThreshold(1, 1), false);
  assert.equal(passesEvidenceThreshold(2, 2), true);
